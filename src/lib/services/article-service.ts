...
export class ArticleService {
  ...
  /**
   * Format tags from query results synchronously
   */
  private static formatTags(articleTags: any[], languageCode: string) {
    ...
    return articleTags.map(at => {
      ...
      tag !== null && tag.name !== 'Unnamed Tag'
    );
  }

  /**
   * Get translated tag names for a given language with fallback to original names
   * @deprecated Use nested include with formatTags for better performance
   */
  ...
  static async getArticleBySlug(slug: string, languageCode: string) {
    ...
    // Get translated tags for this language synchronously
  ...
  static async getPublishedArticles(
    ...
    const processedArticles = articles.map((article: any) => {
      ...
      const translatedTags = this.formatTags((base as any).tags, languageCode);
  ...
  static async getArticlesWithTranslations({
    ...
    if (!articles || total === null) {
      ...
      const processedArticles = (articles as any[] || []).map((article: any) => {
        const translation = article.translations[0];
