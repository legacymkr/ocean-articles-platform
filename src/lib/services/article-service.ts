...
export class ArticleService {
  ...
  /**
   * Format tags with translations synchronously
   * Used to avoid N+1 queries by processing pre-fetched translations
   */
  private static formatTags(articleTags: any[]) {
  ...
  static async getArticleBySlug(slug: string, languageCode: string) {
    ...
    // Get translated tags for this language synchronously from the pre-fetched data
    const translatedTags = this.formatTags((article as any).tags);
  ...
  static async getPublishedArticles(
    ...
    const processedArticles = articles.map((article) => {
      ...
      return {
        ...base,
        tags: this.formatTags((article as any).tags),
  ...
  static async getArticleById(articleId: string, languageId?: string) {
    ...
    // Get translated tags for this language synchronously from pre-fetched data
    const translatedTags = this.formatTags(article.tags);
