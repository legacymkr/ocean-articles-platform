...
export async function GET() {
  try {
    console.log('🗺️ Generating German sitemap...');
    
    const urls = await SitemapService.generateLanguageSitemap('de');
    const sitemapXml = SitemapService.generateXmlSitemap(urls);

    console.log(`✅ Generated German sitemap with ${urls.length} URLs`);
